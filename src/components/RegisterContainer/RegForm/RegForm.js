...
function RegForm() {
  ...
  // 작가 정보등록용
  const [isOtherArea, setIsOtherArea] = useState(
    photographer !== null ? photographer.isOtherArea : false,
  );
  const [activityCity, setActivityCity] = useState(
    photographer.activityCity ? photographer.activityCity : areas[0],
  );
  const [activityAddress, setActivityAddress] = useState(
    photographer.activityAddress ? photographer.activityAddress : '전체',
  );
  const [studioCity, setStudioCity] = useState(
    photographer ? photographer.studioCity : '',
  );
  const [studioAddress, setStudioAddress] = useState(
    photographer ? photographer.studioAddress : '',
  );

  const [hasStudio, setHasStudio] = useState(
    photographer.hasStudio ? photographer.hasStudio : false,
  );
  const [openPostApi, setOpenPostApi] = useState(false);

  const [category, setCategory] = useState(
    photographer.category ? [...photographer.category] : [],
  );
  ...
  const handleRegister = () => {
    ...
    isPhotographer &&
      dispatch(
        auth_actions.registerPhotographer(
          user.userIdx,
          activityCity,
          activityAddress,
          category,
  ...
  return (
    <div>
      <div className={styles.inputContainer}>
          ...
          {categories.map((cat) => (
            <div>
              ...
              <div
                ...
                onClick={() => handleCategoryChange(cat.categoryIdx)}
              >
        ...
        <select
          value={activityCity}
          onChange={(e) => handleActivityCityChange(e.target.value)}
        >
          {areas.map((area) => (
      ...
      <div>
        ...
        <div className={`${styles.radioContainer} ${styles.radio}`}>
          <div>
            <input type="radio" value={true} checked={isOtherArea === true} />
            <div
              className={styles.radioBtn}
              onClick={() => setIsOtherArea(true)}
            >
              <CheckCircleIcon />
            </div>
            <br />
          ...
          <div>
            <input type="radio" value={false} checked={isOtherArea === false} />
            <div
              className={styles.radioBtn}
              onClick={() => setIsOtherArea(false)}
            >
              <CancelIcon />
            </div>
          </div>
      ...
      <div>
        ...
        <div className={`${styles.radioContainer} ${styles.radio}`}>
          <div>
            <input type="radio" value={true} checked={hasStudio} />
            <div
              className={styles.radioBtn}
              onClick={() => handleHasStudioChange(true)}
            >
              <PersonIcon />
              <label>보유</label>
            </div>
          ...
          <div>
            <input type="radio" value={false} checked={hasStudio === false} />
            <div
              className={styles.radioBtn}
              onClick={() => handleHasStudioChange(false)}
            >
              <PersonIcon />
              <label>미보유</label>
            </div>
        ...
        <div>
          <div className={styles.inputContainer}>
            ...
            <input
              ...
              onClick={() => setOpenPostApi(!openPostApi)}
            />
          </div>
          <div className={styles.inputContainer}>
            <label>상세주소</label>
